...
AIを使用する場合は、`.env` に設定した `OPENAI_*` 環境変数を使用する。

ポイント換算レートは`.env`の`POINTS_PER_USD`で指定し、API料金1ドルを何ポイントとして
扱うかを決める。初期値は1700（ドル料金の1.7倍）とする。
...
認証はWebAuthnのパスキー方式を使用する。パスキー登録時のユーザーIDはサーバー側で
生成した128bitの暗号学的乱数をBase62UXでエンコードし、パスキー公開鍵とcredential IDを保存する。
新規登録ではNANI Terms v1.0を表示し、同意をサーバー側で確認してからアカウントを作成する。

ユーザーは以下を基本とする。

```text
id
password_hash（既存schema互換用。認証には使用しない）
is_admin
...
パスキーのchallenge、公開鍵、counterをサーバー側で管理し、認証時に署名とorigin、RP IDを
検証する。WebAuthnの署名検証を独自実装せず、実績のある検証ライブラリを使用する。

## 管理者

`.env`の`ADMIN_USER_ID`と一致するユーザーを管理者として扱う。管理者権限はDBの値を
clientから受け取って決めず、サーバー側で環境変数と認証済みユーザーIDを比較する。

```text
ADMIN_USER_ID=<パスキー登録後に発行されたユーザーID>
```

新規登録時のIDは128bitランダム値をBase62UXで生成する。`ADMIN_USER_ID`が設定されている場合、
未登録の管理者IDを最初のパスキー登録に使用できる。既存DB起動時に既存ユーザーや
パスキーを上書きしてはいけない。

## Login / Session

登録とloginはパスキーで行う。login時はdiscoverable credentialを使用し、IDやpasswordを 入力させない。
...
秘密鍵やパスキー情報をcookieへ保存しない。

## アカウント管理

ログイン後の画面右上に表示するユーザーID部分をタッチすると、アカウント管理画面を 表示する。
...
最後の1つのパスキーは、ログイン不能になるため削除できないようにする。アカウント削除
では、ユーザーに紐づくパスキー、セッション、議事録などのデータを削除し、確認後に ログアウトする。

端末やパスワードマネージャーからのパスキー削除は、アプリ内のパスキー削除とは別操作
である。端末側から削除しても、サーバー側のcredential登録は自動削除しない。

## 権限管理

認証と権限確認を分離して考える。

管理者権限はサーバー側で`.env`の`ADMIN_USER_ID`と認証済みユーザーIDを比較して決める。
clientから送られたuser ID、role、`is_admin`等を信用しない。
